...
#else
#define MOUTH_GRID_N  (MOUTH_GRID_NX*MOUTH_GRID_NY)
#endif
...
int g_iterTimes[6] = {10,9,7,6,4,2};
...
CChildView::CChildView():m_oldPt(0,0)
{
	...
	m_fMorph = 0;
	m_rcML = m_rcMR = cvRect(0,0,0,0);

	m_bShowCornerBox = false;
	m_lcount = 0;
	m_rcount = 0;
...
CChildView::~CChildView()
{
	...
	for(int i=0;i<m_nPyr;i++)
	{
		cvReleaseImage(&m_facepyr[i]);
	}
	if(m_facepyr)
		delete[] m_facepyr;
	m_facepyr = NULL;
...
void CChildView::Draw()
{
	...
	if(!m_bAnimation)
	{
		...
		if(m_nIterMax == 0)
		{
			strItrMax.Format(_T("Max Iter:%s"), _T("Inf"));
		}
		else if(m_nIterMax > 0)
		{
			strItrMax.Format(_T("Max Iter:%d"), m_nIterMax);
		}
		else
		{
			strItrMax.Format(_T("Max Iter:%s"), _T("scheduled"));
		}
		m_mdc.TextOut(0, 80, strItrMax);

		if(m_bUsePyr)
		{
			m_mdc.TextOut(0, 100, "Pyr Detect On");
		}
		else
		{
			m_mdc.TextOut(0, 100, "Pyr Detect Off");
		}

		if(m_bAuto)
		{
			m_mdc.TextOut(0,120,"Auto Detect On");
		}
		else
		{
			m_mdc.TextOut(0,120,"Auto Detect Off");
		}

		if(m_bFit)
		{
			CString strLvl;
			strLvl.Format(_T("Fit success(level:%d)"), m_nDetLevel);
			m_mdc.TextOut(0,140,strLvl);
		}
	}
	else
	{
...
void CChildView::BuildPyramid()
{
	...
	int width = m_fgray->width;
	int height = m_fgray->height;
	int nPyr = 0;

	int pwidth = width;
	int pheight = height;
	while(pwidth%2 == 0 || pheight%2 == 0)
	{
		pwidth /= 2;
		pheight /= 2;
		nPyr++;
	...
	for(int i=0;i<m_nPyr;i++)
	{
		pwidth /= 2;
		pheight /= 2;
		m_facepyr[i] = cvCreateImage(cvSize(pwidth, pheight), IPL_DEPTH_8U, 1);
		IplImage* imgorg = (i==0 ? m_fgray : m_facepyr[i-1]);
		IplImage* imgcur = m_facepyr[i];
		for(int i=0;i<imgcur->height;i++)
		{
			for(int j=0;j<imgcur->width;j++)
			{
				uchar pyr1 = *PTR_PIX(*imgorg, 2*j,   2*i);
				uchar pyr2 = *PTR_PIX(*imgorg, 2*j+1, 2*i);
				uchar pyr3 = *PTR_PIX(*imgorg, 2*j,   2*i+1);
				uchar pyr4 = *PTR_PIX(*imgorg, 2*j+1, 2*i+1);
				*PTR_PIX(*imgcur, j, i) = (uchar)(((int)pyr1+(int)pyr2+(int)pyr3+(int)pyr4)/4);
			}
		}
	}
}
...
#ifdef FACE_DEB
	m_bitmap.CreateCompatibleBitmap(&dc, DIM_FACE_IMG,DIM_FACE_IMG);//1024, 768
#else
...
void CChildView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	case 'P':
		...
		break;
	case 'U':
		m_bShowCornerBox = !m_bShowCornerBox;
		Draw();
		break;
#ifdef FACE_DEB
	case 'M':
...
bool CChildView::Fit()
{
	...
	for(int i=0;i<12;i++)
	{
		...
		*vp[poslist[nparam]]+=s*paramlist[nparam];
		...
		if(W_MOUTH_MIN!=0 || W_MOUTH_MAX!=0)
		{
			if(W>W_MOUTH_MAX)W=W_MOUTH_MAX;
			else if(W<W_MOUTH_MIN)W=W_MOUTH_MIN;
		}
		if(H_MOUTH_MIN!=0 || H_MOUTH_MAX!=0)
		{
			if(H>H_MOUTH_MAX)H=H_MOUTH_MAX;
			else if(H<H_MOUTH_MIN)H=H_MOUTH_MIN;
		}
	...
	if(index!=-1)
	{
		...
		*vparam[poslist[pos]]+=s*paramlist[pos];
		...
		if(W_MOUTH_MIN!=0 || W_MOUTH_MAX!=0)
		{
			if(m_EyeW>W_MOUTH_MAX)m_EyeH=W_MOUTH_MAX;
			else if(m_EyeW<W_MOUTH_MIN)m_EyeW=W_MOUTH_MIN;
		}
		if(H_MOUTH_MIN!=0 || H_MOUTH_MAX!=0)
		{
			if(m_EyeH>H_MOUTH_MAX)m_EyeH=H_MOUTH_MAX;
			else if(m_EyeH<H_MOUTH_MIN)m_EyeH=H_MOUTH_MIN;
		}
		return true;
	}
...
bool CChildView::Fit(FaceData* fitdata, int level)
{
	if(level > m_nPyr)
		return false;
	IplImage* fitimg = NULL;
	...
	{
		fitimg = m_facepyr[level-1];
	}

	float scale = powf(0.5, level);
	float cval=FitEyeLid(fitimg, cvPoint2D32f(fitdata->x*scale,fitdata->y*scale),fitdata->w*scale,fitdata->h*scale,fitdata->c/scale,fitdata->s);
	m_fFitEnrgy = cval;
	float fitval[12];
	int sign[2]={-1,1};
	float X,Y,W,H,Crv,Slp;
	...
	float* vp[6]={&X,&Y,&W,&H,&Crv,&Slp};
	float paramlist[6]={POS_SHIFT/scale, POS_SHIFT/scale, SZ_SHIFT, SZ_SHIFT, CRV_SHIFT, SLP_SHIFT};
	int poslist[6]={0,1,2,3,4,5};
	for(int i=0;i<12;i++)
	{
		int s=sign[i%2];
		...
		*vp[poslist[nparam]]+=s*paramlist[nparam];
		...
		fitval[i]=FitEyeLid(fitimg, cvPoint2D32f(X*scale,Y*scale),W*scale,H*scale,Crv/scale,Slp);
		if(nparam == 0 && s == 1)
		{
	...
	if(nIDEvent == 0)
	{
		if(m_bShowGrid)
		{
			...
			if(m_nIterMax == 0)
				...
				bDoFit = (m_nTime < m_nIterMax);
			else
				bDoFit = (m_nTime < g_iterTimes[m_nDetLevel]);
			if(bDoFit)
			{
				for(int i=0;i<EYE_GRID_NX*EYE_GRID_NY;i++)
				{
					m_bFit|=Fit(&lEGrid[i], m_nDetLevel);
					m_bFit|=Fit(&rEGrid[i], m_nDetLevel);
				}
			}
			m_bFitMove = false;
			Vec2 vdiff[MOUTH_GRID_N];
			float last_ela_energy = 0;
			float last_fit_energy = 0;
			if(!m_bMFit)
			{
				for(int i=0;i<MOUTH_GRID_N;i++)
				{
					vdiff[i]=Vec2(0,0);
					Fit(&MGrid[i], m_nDetLevel);
					last_fit_energy += m_fFitEnrgy*FIT_SCALE;
					for(int j=0;j<MOUTH_GRID_N;j++)
					{
						...
						last_ela_energy += -g_fElaParam[g_elaMat[i][j]]*powf(offset, 2)/2;
#endif
						vdiff[i]+=vmove;
						//MGrid[i].x+=vmove.x;
						//MGrid[i].y+=vmove.y;
					...
					float scale = POS_SHIFT*powf(2, m_nDetLevel);
					if(vdiff[i].length()>scale)
						vdiff[i]=vdiff[i].normalize()*scale;
				}
			}
			last_ela_energy /= 2;
			last_fit_energy += last_ela_energy;

			if(((last_fit_energy > m_fEnergy || m_nTime == 0) && !m_bMFit)
				&& bDoFit)
			{
				m_bFit = true;
				m_fEnergy = last_fit_energy;
...
FaceRegion CChildView::GetDetectedRegion(DetectType type)
{
	...
	for(int i=0;i<dim;i++)
	{
		...
		Insert(facedata.x-facedata.w/2*EDGE_PADDING, xmintop, nxmin,0);
		Insert(facedata.x+facedata.w/2*EDGE_PADDING, xmaxtop, nxmax,1);
...
void CChildView::OneTimeDetect()
{
	...
	while(nLevel >= 0)
	{
		...
		{
			nTime = 0;
			while(Fit(&lEGrid[i], nLevel))
			{
				nTime++;
				if(m_nIterMax > 0 && nTime >= m_nIterMax)
				{
					break;
				}
				else if(m_nIterMax == -1 && nTime >= g_iterTimes[nLevel])
				{
					break;
				}
			}
			nTime = 0;
			while(Fit(&rEGrid[i], nLevel))
			{
				nTime++;
				if(m_nIterMax > 0 && nTime >= m_nIterMax)
				{
	...
	while(true)
	{
		...
		for(int i=0;i<MOUTH_GRID_N;i++)
		{
			...
			vdiff[i]+=FIT_SCALE*m_vFitSt;
			float scale = POS_SHIFT*powf(2, nLevel);
			if(vdiff[i].length()>scale)
				vdiff[i]=vdiff[i].normalize()*scale;
		}
		last_ela_energy /= 2;
		...
		if(m_nIterMax == 0)
			...
			bDoFit = (nTime < m_nIterMax);
		else
			bDoFit = (nTime < g_iterTimes[nLevel]);
		if((last_fit_energy < energy && nTime != 0) || !bDoFit)
		{
			nTime = 0;
			nLevel--;
			if(nLevel < 0)
...
void CChildView::FindCorner()
{
	m_rcML.x = (m_rgnMouth.topleft.x+m_rgnMouth.bottomleft.x)/2-20;
	m_rcML.y = (m_rgnMouth.topleft.y)-10;
	m_rcML.width = 80;
	m_rcML.height = m_rgnMouth.bottomleft.y-m_rgnMouth.topleft.y+20;

	m_rcMR.x = (m_rgnMouth.topright.x+m_rgnMouth.bottomright.x)/2-60;
	m_rcMR.y = (m_rgnMouth.topright.y)-10;
	m_rcMR.width = 80;
	m_rcMR.height = m_rgnMouth.bottomright.y-m_rgnMouth.topright.y+20;

	if(m_rcML.x<0)m_rcML.x=0;
	if(m_rcML.x+m_rcML.width>m_face->width)m_rcML.width=m_face->width-m_rcML.x;
	if(m_rcML.x>m_face->width)m_rcML.x=m_rcML.width=0;

	if(m_rcML.y<0)m_rcML.y=0;
	if(m_rcML.y+m_rcML.height>m_face->height)m_rcML.height=m_face->height-m_rcML.y;
	if(m_rcML.y>m_face->height)m_rcML.y=m_rcML.height=0;

	if(m_rcMR.x<0)m_rcMR.x=0;
	if(m_rcMR.x+m_rcMR.width>m_face->width)m_rcMR.width=m_face->width-m_rcMR.x;
	if(m_rcMR.x>m_face->width)m_rcMR.x=m_rcMR.width=0;

	if(m_rcMR.y<0)m_rcMR.y=0;
	if(m_rcMR.y+m_rcMR.height>m_face->height)m_rcMR.height=m_face->height-m_rcMR.y;
	if(m_rcMR.y>m_face->height)m_rcMR.y=m_rcMR.height=0;

	m_lcount=5;
	FindMouthCorner(m_rcML, m_lcorner, &m_lcount);
	m_rcount=5;
	FindMouthCorner(m_rcMR, m_rcorner, &m_rcount);
}

void CChildView::FindMouthCorner(CvRect rcCorner, CvPoint2D32f* pt, int* count)
{
	if(!(rcCorner.x==0&&rcCorner.width==0)&&!(rcCorner.y==0&&rcCorner.height==0))
	{
		IplImage* imgcorner = cvCreateImage(cvSize(rcCorner.width/2, rcCorner.height/2), IPL_DEPTH_8U, 1);
		IplImage* eigen = cvCreateImage(cvGetSize(imgcorner), IPL_DEPTH_32F, 1);
		IplImage* temp = cvCreateImage(cvGetSize(imgcorner), IPL_DEPTH_32F, 1);
		for(int i=0;i<imgcorner->height;i++)
		{
			for(int j=0;j<imgcorner->width;j++)
			{
				int p1=*(uchar*)PTR_PIX(*m_fgray, rcCorner.x+j*2, rcCorner.y+i*2);
				int p2=*(uchar*)PTR_PIX(*m_fgray, rcCorner.x+j*2+1, rcCorner.y+i*2);
				int p3=*(uchar*)PTR_PIX(*m_fgray, rcCorner.x+j*2, rcCorner.y+i*2+1);
				int p4=*(uchar*)PTR_PIX(*m_fgray, rcCorner.x+j*2+1, rcCorner.y+i*2+1);
				*PTR_PIX(*imgcorner, j, i) = (uchar)((p1+p2+p3+p4)/4);
			}
		}
		cvGoodFeaturesToTrack(imgcorner, eigen, temp, pt, count, 0.1, 2, NULL, 5);
		for(int i=0;i<*count;i++)
		{
			pt[i].x=pt[i].x*2+rcCorner.x;
			pt[i].y=pt[i].y*2+rcCorner.y;
		}
...
void CChildView::DrawCornerBox(CDC* pDC)
{
	FindCorner();
	CPen bPen, yPen;
	...
	pDC->MoveTo(m_rcML.x,m_rcML.y);
	pDC->LineTo(m_rcML.x+m_rcML.width, m_rcML.y);
	pDC->LineTo(m_rcML.x+m_rcML.width, m_rcML.y+m_rcML.height);
	pDC->LineTo(m_rcML.x, m_rcML.y+m_rcML.height);
	pDC->LineTo(m_rcML.x, m_rcML.y);

	pDC->MoveTo(m_rcMR.x,m_rcMR.y);
	pDC->LineTo(m_rcMR.x+m_rcMR.width, m_rcMR.y);
	pDC->LineTo(m_rcMR.x+m_rcMR.width, m_rcMR.y+m_rcMR.height);
	pDC->LineTo(m_rcMR.x, m_rcMR.y+m_rcMR.height);
	pDC->LineTo(m_rcMR.x, m_rcMR.y);

	pDC->SelectObject(&yPen);

	for(int i=0;i<m_lcount;i++)
	{
		pDC->SetPixel(m_lcorner[i].x, m_lcorner[i].y,RGB(255,255,0));
//		pDC->MoveTo(m_lcorner[i].x, m_lcorner[i].y);
//		pDC->LineTo(m_lcorner[i].x, m_lcorner[i].y);

		pDC->SetPixel(m_rcorner[i].x, m_rcorner[i].y,RGB(255,255,0));
//		pDC->MoveTo(m_rcorner[i].x, m_rcorner[i].y);
//		pDC->LineTo(m_rcorner[i].x, m_rcorner[i].y);
	}
