...
void DefaultPixelFunc(RawImage* image, Point2D pos, int count, float* vdata, void* p)
{
	...
	{
		pMeshInRender->m_pPShader(pos, pClr, pTex, pUser, &colorout, pDep, pMeshInRender->m_iFace, pMeshInRender->m_pUserPtr);
		colorout*=255;
	}
...
CMesh::CMesh()
{
	...
	m_nFaceAttrLen = 0;
	m_FaceAttr = NULL;
	ZeroMemory(m_szFaceAttrDesc, sizeof(m_szFaceAttrDesc));
	m_nGrpAttrLen = 0;
	m_nGrpAttr = 0;
	m_GrpAttr = NULL;
	ZeroMemory(m_szGrpAttrDesc, sizeof(m_szGrpAttrDesc));
...
bool CMesh::LoadMesh(char* filename)
{
	...
	if(m_FaceAttr)
		delete[] m_FaceAttr;
	int cb = 0;
	ReadFile(hfile, &cb, 4, &nRd, 0);
	if(nRd>0 && cb>0)
	{
		BYTE* buf = new BYTE[cb];
		ReadFile(hfile, buf, cb, &nRd, 0);
		if((int)nRd<cb)
		{
			delete[] buf;
			return false;
		...
		m_nFaceAttrLen = *(int*)cur;
		cur+=4;

		cnt+=sizeof(m_szFaceAttrDesc);
		if(cnt>cb)
		{
			delete[] buf;
			return false;
		}
		memcpy(m_szFaceAttrDesc, cur, sizeof(m_szFaceAttrDesc));
		cur+=sizeof(m_szFaceAttrDesc);

		cnt+=m_nFaceAttrLen*m_nFace*4;
		if(cnt>cb)
		{
			delete[] buf;
			return false;
		}
		m_FaceAttr = new float[m_nFaceAttrLen*m_nFace];
		memcpy(m_FaceAttr, cur, m_nFaceAttrLen*m_nFace*4);
		delete[] buf;
	}
	if(m_GrpAttr)
		delete[] m_GrpAttr;
	...
	if(nRd>0 && cb>0)
	{
		...
		m_nGrpAttr = *(int*)cur;
		m_nGrpAttrLen = *(int*)(cur+4);
		cur+=8;

		cnt+=sizeof(m_szGrpAttrDesc);
		if(cnt>cb)
		{
			delete[] buf;
			return false;
		}
		memcpy(m_szGrpAttrDesc, cur, sizeof(m_szGrpAttrDesc));
		cur+=sizeof(m_szGrpAttrDesc);

		cnt+=m_nGrpAttrLen*m_nGrpAttr*4;
		if(cnt>cb)
		{
			delete[] buf;
			return false;
		}
		m_GrpAttr = new float[m_nGrpAttrLen*m_nGrpAttr];
		memcpy(m_GrpAttr, cur, m_nGrpAttrLen*m_nGrpAttr*4);
		delete[] buf;
	}
	CloseHandle(hfile);
	return true;
...
bool CMesh::SaveMesh(char* filename, int indexFormat)
{
	...
	if(m_nFaceAttrLen>0)
	{
		int cb = 4+sizeof(m_szFaceAttrDesc)+m_nFaceAttrLen*m_nFace*4;
		WriteFile(hfile, &cb, 4, &nWt, 0);
		WriteFile(hfile, &m_nFaceAttrLen, 4, &nWt, 0);
		WriteFile(hfile, m_szFaceAttrDesc, sizeof(m_szFaceAttrDesc), &nWt, 0);
		WriteFile(hfile, m_FaceAttr, m_nFaceAttrLen*m_nFace*4, &nWt, 0);
	}
	if(m_nGrpAttr>0 && m_nGrpAttrLen>0)
	{
		int cb = 8+sizeof(m_szGrpAttrDesc)+m_nGrpAttr*m_nGrpAttrLen*4;
		WriteFile(hfile, &cb, 4, &nWt, 0);
		WriteFile(hfile, &m_nGrpAttr, 4, &nWt, 0);
		WriteFile(hfile, &m_nGrpAttrLen, 4, &nWt, 0);
		WriteFile(hfile, m_szGrpAttrDesc, sizeof(m_szGrpAttrDesc), &nWt, 0);
		WriteFile(hfile, m_GrpAttr, m_nGrpAttr*m_nGrpAttrLen*4, &nWt, 0);
	}
	CloseHandle(hfile);
	return true;
}
...
int CMesh::GetFaceAttrLen()
{
	return m_nFaceAttrLen;
}

float* CMesh::GetFaceAttr(int idface)
{
	if(!m_FaceAttr)
		return NULL;
	if(idface<0||idface>=m_nFace)
		return NULL;
	return m_FaceAttr+m_nFaceAttrLen*idface;
}

char* CMesh::GetFaceAttrDesc()
{
	return m_szFaceAttrDesc;
}

int CMesh::GetGrpAttrCount()
{
	return m_nGrpAttr;
}

int CMesh::GetGrpAttrLen()
{
	return m_nGrpAttrLen;
}

float* CMesh::GetGrpAttr(int idGrp)
{
	if(idGrp<0 || idGrp>=m_nGrpAttr)
		return NULL;
	return m_GrpAttr+m_nGrpAttrLen*idGrp;
}

char* CMesh::GetGrpAttrDesc()
{
	return m_szGrpAttrDesc;
}