...
void Triangle2D(RawImage image, Vec2 vert[3], int count, float (*vdata)[3], PIXELFUNC pFunc, void* p, Rect2D* pVp)
{
	Vec3 vert3d[3];
	for(int i=0;i<3;i++)
	{
	...
	Triangle(image, vert3d, count, vdata, pFunc, p, &Vp3d);
}

void Triangle(RawImage image, Vec3 vert[3], int count, float (*vdata)[3], PIXELFUNC pFunc, void* p, Rect2D* pVp, int nCullMode)
{
	Vec3 vbuf[3];
	int i;
	int wind = 1;
	...
	if(nCullMode == CULL_MODE_CCW && wind == 1)
		return;
	if(nCullMode == CULL_MODE_CW && wind == -1)
		return;
	...
	for(y=max(ymin,0); y<min(ymax,image.height); y++)
	{
		...
		float edge[4]={e12,e13,e23,(float)((xbd-vpx)/fabs(scale))};
		...
		for(int x=max(xbuf[0],0); x<min(xbuf[1],image.width); x++)
		{
			if(pFunc == NULL)
			{
				for(int k=0;k<image.nChannels;k++)
				{
					...
					switch((unsigned int)(image.depth))
					{
					case IPL_DEPTH_8U:
						*ptr+=80;
						break;
			...
			else
			{
				...
				Vec2 point((x+0.5-vpx)/fabs(scale), -(y+0.5-vpy)/scale);
				...
				pFunc(&image, Point2D(x,y), count, pdata, p);
			}
		}
	}
	if(pdata != NULL)
...
Vec3 Sample(RawImage image, Vec2 tex, int wrapType, int filterMode, bool bUniformCoord, Vec3 bordercolor)
{
	if(bUniformCoord)
	{
		tex.x*=image.width;
	...
	for(int k=0;k<szKernel;k++)
	{
		for(int l=0;l<szKernel;l++)
		{
			...
			if(ntx<0||ntx>=image.width||nty<0||nty>=image.height)
			{
				...
				{
					ntx=fabs(fabs(fmodf(ntx,(2*image.width-1)))-(image.width-1));
					nty=fabs(fabs(fmodf(nty,(2*image.height-1)))-(image.height-1));
				}
			}
...
Vec4 SampleEx(RawImage image, Vec2 tex, int wrapType, int filterMode, bool bUniformCoord, Vec4 bordercolor)
{
	if(bUniformCoord)
	{
		tex.x*=image.width;
	...
	for(int k=0;k<szKernel;k++)
	{
		for(int l=0;l<szKernel;l++)
		{
			...
			for(int p=0;p<min(4,image.nChannels);p++)
			{
				uchar* ptr = PTR_PIX(image,ntx,nty)+p*((image.depth&0x7fffffff)>>3);
				switch((unsigned int)(image.depth))
				{
