...
{
	// TODO: Add your message handler code here and/or call default
	static Vec2 posDetM[10][MOUTH_GRID_N];
	...
	if(nIDEvent == 0)
	{
		if(m_bShowGrid)
		{
			...
			{
				vdiff[i]=Vec2(0,0);
				//fitval[i] = FitEyeLid(m_fgray, cvPoint2D32f(MGrid[i].x,MGrid[i].y),MGrid[i].w,MGrid[i].h,MGrid[i].c,MGrid[i].s);
				/*m_bFit|=*/Fit(&MGrid[i]);
			...
			for(int i=0;i<MOUTH_GRID_N;i++)
			{
				MGrid[i].x+=vdiff[i].x;
				MGrid[i].y+=vdiff[i].y;
				int index = m_nTime%10;
				posDetM[index][i].x=MGrid[i].x;
				posDetM[index][i].y=MGrid[i].y;
				if(m_nTime<10)m_bFit = true;
				else
				{
					Vec2 vcrit = posDetM[index][i]-posDetM[(index+1)<10?index+1:0][i];
					//float fitval2 = FitEyeLid(m_fgray, cvPoint2D32f(MGrid[i].x,MGrid[i].y),MGrid[i].w,MGrid[i].h,MGrid[i].c,MGrid[i].s);
					m_bFit|=(vcrit.length()>3);//(vdiff[i].length()>MIN_ITER_CRIT);
					//m_bFit|=(fitval[i]<fitval2);
				}
			}
			m_nTime++;
			m_bFitMove = true;
...
#define MAHALANOBIS_COEFFX  1.0
#define MAHALANOBIS_COEFFY  0.1
	...
	do 
	{
		...
		for(int i=0;i<n;i++)
		{
			if(cls[i])
			{
				sum.x+=val[i].x;
				sum.y+=val[i].y;
				sqsum+=pow(val[i].x/MAHALANOBIS_COEFFX,2)+pow(val[i].y/MAHALANOBIS_COEFFY,2);
				cnt++;
			}
		}
		float meanx=sum.x/cnt;
		float meany=sum.y/cnt;
		double stddev=sqrt(sqsum/cnt-pow(meanx/MAHALANOBIS_COEFFX,2)-pow(meany/MAHALANOBIS_COEFFY,2));
		for(int i=0;i<n;i++)
		{
			if(sqrt(pow((val[i].x-meanx)/MAHALANOBIS_COEFFX,2)+pow((val[i].y-meany)/MAHALANOBIS_COEFFY,2))>max(CLASS_THRESH*stddev,10))
			{
				if(cls[i])
				{
					cls[i]=false;
