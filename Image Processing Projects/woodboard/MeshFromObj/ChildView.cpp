...
int CChildView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_fore=cvLoadImage("wood1.jpg");
	m_bk=cvLoadImage("wood2.jpg");
	...
	m_mask=cvCreateImage(cvGetSize(m_ImgBk1),IPL_DEPTH_8U,1);
	cvZero(m_mask);

	for(int i=0;i<m_mask->height;i++)
	{
		for(int j=0;j<m_mask->width;j++)
		{
			float y=(float)i/m_mask->height;
			float dx=sin(y*5*PI);
			float x=(float)j/m_mask->width;
			x+=dx*0.05;
			float h=fabs(fmod(x*300,2)-1);
			*PTR_PIX(*m_mask, j,i)=255*h;
		}
	}
