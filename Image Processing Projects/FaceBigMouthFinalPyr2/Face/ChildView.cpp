...
void CChildView::Draw()
{
	...
	if(m_bShowDetRegion)
	{
		...
		FaceRegion rgnface = m_Detector.GetDetRegion(Det_Face);
		float x = rgnface.topleft.x;
		float y = rgnface.topleft.y;
		float width = rgnface.topright.x - rgnface.topleft.x;
		float height = rgnface.bottomleft.y - rgnface.topleft.y;
		CvSize sz = m_Detector.GetDestSize(DEST_TYPE_EXPRESSION);
		for(int i=0;i<3;i++)
		{
			FaceRegion faceregion = m_Detector.GetDetRegion(DetType[i]);
			CvPoint2D32f* pt[4] = {&faceregion.topleft, &faceregion.topright,
				&faceregion.bottomleft, &faceregion.bottomright};
			for(int i=0;i<4;i++)
			{
				pt[i]->x = (pt[i]->x - x)/width*sz.width;
				pt[i]->y = (pt[i]->y - y)/height*sz.height;
			}
