...
void CFaceDetector::BuildPyramid()
{
	if(!m_fgray)
		return;
...
void CFaceDetector::FitFace()
{
	...
	while(true)
	{
		...
		if((last_fit_energy < energy && nTime != 0)
			|| (m_nIterMax > 0 && nTime >= m_nIterMax))
		{
			nTime = 0;
			nLevel--;
			if(nLevel < 0)
...
bool CFaceDetector::Fit(FaceData* fitdata, int level)
{
	if(level > m_nPyr)
		return false;
