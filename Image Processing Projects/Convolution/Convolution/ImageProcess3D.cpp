...
void Triangle(RawImage image, Vec3 vert[3], int count, float (*vdata)[3], PIXELFUNC pFunc, void* p, Rect2D* pVp, int nCullMode)
{
	...
	for(y=max(ymin,0); y<min(ymax,image.height); y++)
	{
		...
		float edge[4]={e12,e13,e23,(xbd-vpx)/scale};
		...
		{
			xbuf[order] = R_SAMP(e13*scale+vpx);
			xbuf[1-order] = xbd;
		}
		else
		{
...
Vec4 SampleEx(RawImage image, Vec2 tex, int wrapType, int filterMode, bool bUniformCoord)
{
	if(bUniformCoord)
	{
		tex.x*=image.width;
	...
	for(int k=0;k<szKernel;k++)
	{
		for(int l=0;l<szKernel;l++)
		{
			...
			if(ntx<0||ntx>=image.width||nty<0||nty>=image.height)
			{
				if(wrapType == WRAP_TYPE_BORDER)
				{
					color[k][l]=Vec4(128,128,128,128);
					continue;
				}
				else if(wrapType == WRAP_TYPE_CLAMP)
				{
